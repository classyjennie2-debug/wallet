...
export const LandingPageV2 = () => {
  ...
  return (
    <div className="min-h-screen w-full overflow-hidden bg-[var(--background)] text-[var(--foreground)]">
      ...
      <div className="relative z-10">
        <header className="sticky top-0 z-40 border-b border-surface bg-surface/95 shadow-xl shadow-slate-200/40 backdrop-blur-xl">
          <div className="mx-auto flex w-full max-w-7xl flex-col gap-3 px-4 py-3 sm:px-6">
            ...
            <nav className="flex flex-wrap items-center justify-center gap-2 text-xs text-muted sm:justify-end sm:text-sm">
        ...
        <section className="mx-auto flex w-full max-w-6xl flex-col gap-10 px-5 py-16 sm:px-6 sm:py-24">
          <div className="w-full rounded-[32px] border border-surface bg-surface shadow-[0_40px_120px_-70px_rgba(15,23,42,0.08)] p-8 sm:p-12">
            <div className="grid gap-8 xl:grid-cols-[1.2fr_0.8fr] lg:items-center">
              <div className="space-y-8 min-w-0">
                ...
                <div className="grid gap-3 sm:grid-cols-3">
                  {[
                    ...
                    <div key={item} className="rounded-3xl border border-surface bg-surface-muted px-4 py-3 text-sm font-medium text-foreground shadow-sm">
                      {item}
                    </div>
                  ))}
                </div>

                <div className="grid gap-3 rounded-[28px] border border-surface bg-surface-muted p-5 text-sm text-muted sm:grid-cols-2">
                  {[
                    ...
                    <div key={item} className="rounded-3xl bg-surface px-4 py-3 text-foreground min-w-0 shadow-sm">
                      {item}
                    </div>
                  ))}
                </div>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  {[
                    ...
                    <div key={item.label} className="rounded-3xl border border-surface bg-surface p-5 transition hover:border-cyan-300 hover:bg-cyan-50">
              ...
              <div className="relative overflow-hidden rounded-[32px] border border-surface bg-surface p-6 shadow-[0_30px_80px_-40px_rgba(15,23,42,0.08)]">
                ...
                <div className="relative z-10">
                  <div className="mb-6 flex items-center justify-between rounded-3xl bg-surface-muted px-4 py-3 text-xs uppercase tracking-[0.25em] text-muted ring-1 ring-surface">
                  ...
                  <div className="rounded-[28px] border border-surface bg-surface-muted p-5 shadow-sm">
                    <div className="mb-5 flex flex-col gap-3 rounded-3xl bg-surface px-4 py-3 shadow-sm sm:flex-row sm:items-center sm:justify-between">
                      <div className="min-w-0">
                        <p className="text-xs uppercase tracking-[0.24em] text-muted">Connected wallet</p>
                        <p className="mt-2 truncate text-lg font-semibold text-foreground">0xA3...F1D2</p>
                      </div>
                      <div className="rounded-3xl bg-surface px-3 py-2 text-xs font-semibold uppercase tracking-[0.24em] text-foreground shadow-sm">High trust</div>
                    </div>
                    ...
                    <div className="grid gap-3 sm:grid-cols-3">
                      ...
                      <div className="rounded-3xl bg-surface p-4 shadow-sm">
                        <p className="text-xs uppercase tracking-[0.25em] text-muted">Approvals</p>
                        <p className="mt-3 text-xl font-semibold text-foreground">4 active</p>
                      </div>
                    ...
                    <div className="mt-5 rounded-[24px] border border-surface bg-surface p-4">
                      <div className="flex flex-col gap-2 text-xs uppercase tracking-[0.24em] text-muted sm:flex-row sm:items-center sm:justify-between">
                        <span>Approval review</span>
                        <span className="text-muted">3 flagged</span>
                      </div>
                      <div className="mt-4 grid gap-3">
                        {[
                          ...
                          <div key={item.title} className="flex flex-col gap-2 rounded-2xl bg-surface-muted px-3 py-3 text-sm text-foreground sm:flex-row sm:items-center sm:justify-between">
                            <div className="min-w-0">
          ...
          <section className="rounded-[32px] border border-surface bg-surface p-8 shadow-2xl shadow-slate-200/40 sm:p-10">
            <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <h2 className="mt-3 text-3xl font-semibold text-foreground">Trust signals that are easier to verify than marketing claims</h2>
              </div>
              <div className="hidden rounded-full bg-surface-muted px-4 py-2 text-sm text-muted sm:block">Built to communicate risk, not just style</div>
            </div>

            <div className="mt-8 grid gap-4 sm:grid-cols-4">
              {principles.map((principle) => (
                <div key={principle.name} className="rounded-3xl border border-surface bg-surface p-5 shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
                  <div className="inline-flex rounded-2xl bg-gradient-to-r from-cyan-500 to-sky-500 px-3 py-2 text-xs font-semibold uppercase tracking-[0.24em] text-foreground">
                    {principle.name}
                  </div>
                  <p className="mt-4 text-sm text-foreground">{principle.detail}</p>
                </div>
              ))}
            </div>
          </section>
            ...
            <div className="grid gap-6 text-center md:grid-cols-3">
              {stats.map((stat) => (
                <div key={stat.label} className="rounded-3xl border border-surface bg-surface p-6 shadow-[0_20px_60px_-45px_rgba(56,189,248,0.15)] transition hover:-translate-y-1 hover:border-cyan-300">
                  ...
                  <p className="mt-2 text-foreground">{stat.label}</p>
                </div>
              ))}
            </div>
          </section>
